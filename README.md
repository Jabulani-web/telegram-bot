...
Run `npm run typecheck` for a no-emit TypeScript check, `npm test` for the build and notification-format tests (including deterministic fuzz cases) plus the secret-scanner tests, `npm run scan:secrets` for the same secret scan CI runs, or `npm run build` to produce the production output.
